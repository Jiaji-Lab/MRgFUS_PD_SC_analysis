...
def reg_aladin(aladin_path, ref_path, src_path, to_path, aff_out_path):
    aladin_skeleton = '{} -ref {} -flo {} -res {} -aff {}'
    reg_cmd = aladin_skeleton.format(aladin_path, ref_path, src_path, to_path, aff_out_path)
    os.system(reg_cmd)

def reg_resample(resample_path, ref_path, src_path, to_path, aff_in_path):
    resample_skeleton = '{} -ref {} -flo {} -res {} -trans {}'
    reg_cmd = resample_skeleton.format(resample_path, ref_path, src_path, to_path, aff_in_path)
    os.system(reg_cmd)